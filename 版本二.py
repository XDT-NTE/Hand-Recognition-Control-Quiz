...
pygame.init()
# 启用高清渲染（抗锯齿）
pygame.display.set_mode((800, 600), pygame.RESIZABLE | pygame.SCALED | pygame.HWSURFACE)
# 获取真实屏幕分辨率
...
def get_option_areas():
    ...
    return {
        "A": (int(w * 0.1), int(h * 0.67), int(w * 0.25), int(h * 0.13)),
        "B": (int(w * 0.38), int(h * 0.67), int(w * 0.25), int(h * 0.13)),
        "C": (int(w * 0.65), int(h * 0.67), int(w * 0.25), int(h * 0.13))
    }
...
def get_font_sizes():
    ...
    return {
        "big": int(60 * scale), "title": int(40 * scale), "question": int(32 * scale),
        "option": int(30 * scale), "info": int(26 * scale), "count": int(28 * scale), "result": int(36 * scale)
    }
...
def draw_answer_interface(surf):
    ...
    w, h = settings["window_size"]
    surf.fill(WHITE)
    title = fonts["title"].render("NTE 体测答题" if settings["language"] == "zh" else "NTE PE Test Quiz", True, BLUE)
    surf.blit(title, title.get_rect(center=(w // 2, int(h * 0.07))))
    q = QUESTIONS[current_question]
    q_txt = fonts["question"].render(q["question"], True, BLACK)
    surf.blit(q_txt, q_txt.get_rect(center=(w // 2, int(h * 0.25))))
    time_color = RED if left_time <= 5 else BLACK
    time_txt = fonts["info"].render(
        f"剩余时间：{int(left_time)}秒" if settings["language"] == "zh" else f"Time left: {int(left_time)}s", True,
        time_color)
    surf.blit(time_txt, (int(w * 0.7), int(h * 0.15)))
    score_txt = fonts["info"].render(f"得分：{score}" if settings["language"] == "zh" else f"Score: {score}", True,
                                     GREEN)
    surf.blit(score_txt, (int(w * 0.1), int(h * 0.15)))
    areas = get_option_areas()
    for opt in q["options"]:
        ...
        t = fonts["option"].render(opt, True, BLACK)
        surf.blit(t, t.get_rect(center=(x + w // 2, y + h // 2 - 10)))
        cnt = fonts["count"].render(
            f"人数：{option_hand_count[k]}" if settings["language"] == "zh" else f"People: {option_hand_count[k]}", True,
            (255, 100, 0))
        surf.blit(cnt, (x + 10, y + h - 30))
