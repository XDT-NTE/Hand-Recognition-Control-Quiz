...
import queue  # 新增：线程间通信
# 题库
QUESTIONS_ZH = [
    {"question": "使用电脑时，不小心打翻水杯洒到键盘上，第一时间应该？", "options": ["A.继续用电脑", "B.立刻拔掉电源", "C.用纸巾擦"], "answer": "B"},
    {"question": "下列哪个是计算机的输入设备？", "options": ["A.显示器", "B.键盘", "C.音箱"], "answer": "B"},
    {"question": "在电脑上画图画时，想要撤销上一步操作，通常按哪个快捷键？", "options": ["A.Ctrl+Z", "B.Ctrl+C", "C.Ctrl+V"], "answer": "A"},
    {"question": "小学生使用电脑时，连续看屏幕最好不超过多久休息一次？", "options": ["A.2小时", "B.20分钟", "C.1小时"], "answer": "B"},
    {"question": "下列哪种行为符合网络安全规范？", "options": ["A.随便点开陌生链接", "B.设置复杂的密码", "C.把密码告诉同学"], "answer": "B"}
]
QUESTIONS_ENG = [
    ...
    {"question": "Which behavior complies with network security norms?",
     "options": ["A.Click on unfamiliar links casually", "B.Set a complex password", "C.Tell classmates your password"], "answer": "B"}
]
# ---------------------- 初始化全局参数 ----------------------
pygame.init()
# 初始化音频模块（关键）
...
# 新增：后台初始化摄像头的函数
# 修改 init_camera_background 函数
def init_camera_background():
    """后台线程初始化摄像头 - 自动检测可用摄像头"""
    global camera_ready, camera_init_error
    try:
        # 尝试多个摄像头索引
        cap = None
        for camera_index in [0]:  # 尝试索引0、1
            ...
            if temp_cap.isOpened():
                ...
                if ret:
                    cap = temp_cap
                    print(f"成功使用摄像头索引: {camera_index}")
                    break
                else:
                    temp_cap.release()
            else:
...
def load_sound(file_path, volume=0.7):
    ...
    try:
        ...
        print(f"提示：未找到音频文件 {file_path}，跳过加载")
        return None
...
def load_bg_music(file_path, volume=0.5):
    ...
    try:
        ...
        print(f"提示：未找到背景音乐文件 {file_path}，跳过播放")
        return False


# 音频文件路径（你可以替换成自己的音频文件路径）
SOUND_CORRECT = load_sound("sounds/correct.mp3")  # 答对音效
SOUND_WRONG = load_sound("sounds/wrong.mp3")  # 答错音效
SOUND_PROGRESS = load_sound("sounds/progress.mp3")  # 进度条填充音效
SOUND_TIME_OUT = load_sound("sounds/timeout.wav")  # 超时音效
SOUND_VICTORY = load_sound("sounds/victory.mp3", 0.8)  # 胜利（全对）音效
SOUND_DEFEAT = load_sound("sounds/defeat.mp3", 0.8)  # 失败（错题多）音效

# 加载背景音乐（优先加载bg_music.mp3，没有则跳过）
...
pygame.display.set_caption("手势交互答题系统")
...
def draw_loading_screen():
    ...
    if camera_init_error:
        loading_text = font.render(f"摄像头初始化失败：{camera_init_error}", True, RED)
    elif camera_ready:
        loading_text = font.render("摄像头已就绪，即将进入系统...", True, GREEN)
    else:
        loading_text = font.render("系统加载中...（正在初始化摄像头）", True, BLUE)

    text_rect = loading_text.get_rect(center=(loading_center[0], loading_center[1] + 50))
    screen.blit(loading_text, text_rect)

    # 绘制版权/提示文字
    hint_text = small_font.render("手势交互答题系统 v1.0", True, BLACK)
...
# 显示加载界面直到初始化完成（修改：等待摄像头就绪）
start_time = time.time()
while loading_screen:
    # 控制加载界面至少显示1.5秒，且等待摄像头就绪
...
def draw_circle_option(surf, center, r, text, progress, is_selected, color=BLUE):
    ...
    if progress >= 50 and not progress_sound_played and SOUND_PROGRESS:
        ...
        progress_sound_played = False  # 进度回落时重置标记
...
def draw_back_circle(surf, delta_time, palms):
    ...
    if touched:
        ...
        progress_sound_played = False  # 离开时重置
...
def draw_answer_interface(surf):
    ...
    # ====================== 标题 ======================
    title = fonts["title"].render("计算机基础答题", True, BLUE)
...
def draw_setting_interface(surf, palms, delta_time):
    ...
    if touched:
        ...
        progress_sound_played = False
    draw_circle_option(screen, c1, r, "小屏", setting_progress["window_small"], sel_win_small)
    ...
    if touched:
        ...
        progress_sound_played = False
    draw_circle_option(screen, c2, r, "中屏", setting_progress["window_mid"], sel_win_mid)
    ...
    if touched:
        ...
        progress_sound_played = False
    draw_circle_option(screen, c3, r, "全屏", setting_progress["window_full"], sel_win_full)
    ...
    if touched:
        ...
        progress_sound_played = False
    draw_circle_option(screen, c4, r, "蓝", setting_progress["color_blue"], sel_blue, BLUE)
    ...
    if touched:
        ...
        progress_sound_played = False
    draw_circle_option(screen, c5, r, "绿", setting_progress["color_green"], sel_green, GREEN)
    ...
    if touched:
        ...
        progress_sound_played = False
    draw_circle_option(screen, c6, r, "红", setting_progress["color_red"], sel_red, RED)
    ...
    if touched:
        ...
        progress_sound_played = False
    draw_circle_option(screen, c7, r, "中文", setting_progress["lang_zh"], sel_zh)
    ...
    if touched:
        ...
        progress_sound_played = False
    draw_circle_option(screen, c8, r, "英文", setting_progress["lang_eng"], sel_eng)
...
while running:
    ...
    if current_state == STATE_START:
        ...
        fonts = get_fonts()
        w, h = settings["window_size"]
        nte = fonts["big"].render("手势交互答题系统", True, BLUE)
        ...
        for c in function_circles:
            ...
            if c["progress"] >= PROGRESS_FULL:
                ...
                progress_sound_played = False  # 重置音效标记
        ...
        if now - result_show_time > 10:
            ...
            answer_history = []
            show_result = False
            victory_defeat_played = False  # 重置胜利失败音效标记
